...
export class DepartmentService {
	...
	async reviewAchievement(id: string, approve: boolean) {
		...
		const achievement = await this.achievementRepo.findOne({ where: { achievement_id: id } });
		if (!achievement) {
	...
	async checkAchievementBelongsToDepartment(achievementId: string, reviewerId: string) {
		const sql = `
			SELECT 1
			FROM achievement a
			...
			LIMIT 1
		`;
		const result = await this.dataSource.query(sql, [achievementId, reviewerId]);
