    ...
    <div v-else-if="achievement" class="detail-card">
      ...
      <div class="info-grid">
        ...
        <div class="info-item">
          ...
          <div class="val"><span class="tag">{{ achievement.category }}</span></div>
        </div>
...
.page-header { width: 100%; max-width: 800px; margin-bottom: 20px; display: flex; align-items: center; gap: 20px; }
.btn-back { background: transparent; border: none; color: var(--secondary-color); font-size: 1rem; cursor: pointer; }
...
.detail-card {
  ...
  border-radius: 24px; box-shadow: 0 10px 30px rgba(0,0,0,0.03); border: 1px solid rgba(0,0,0,0.02);
}

.detail-header { display: flex; justify-content: space-between; align-items: center; }
.student-info { display: flex; align-items: center; gap: 15px; }
.avatar { width: 50px; height: 50px; background: #EBEBE8; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 600; color: var(--secondary-color); font-size: 1.2rem; }
.student-name { margin: 0; font-size: 1.2rem; color: var(--text-color); }
.student-id { color: #888; font-size: 0.9rem; }
.status-badge { padding: 6px 12px; border-radius: 6px; font-size: 0.85rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
.status-badge.pending { background:  #6b8c8b; color: #f6f6f6; }
...
.val { font-size: 1rem; color: var(--text-color); line-height: 1.5; }
.title-text { 
  margin: 0;
  color: var(--accent-color);
  font-size: 1.3rem; }
.tag { background: #f0f0f0; padding: 4px 10px; border-radius: 6px; font-size: 0.9rem; color: #555555; }
.desc-box { background: #f9f9f9; padding: 15px; border-radius: 8px; border: 1px solid #eee; white-space: pre-wrap; }
.file-link { color: var(--primary-color); text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 5px; }
...
.btn-action.reject {
  background: transparent; border: 1px solid #ddd; color: #666;font-size: 1rem;
  padding: 12px 24px; border-radius: 10px; font-weight: 600; cursor: pointer;
}
.btn-action.reject:hover { background: #f5f5f5; }
...
.btn-action.approve:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(125, 157, 156, 0.4); }
</style>